...
ori = f'/mnt/dataset1/MV_Restoration/hypersim/data/ai_001_002/_detail/cam_00/camera_keyframe_orientations.hdf5'
pos = f'/mnt/dataset1/MV_Restoration/hypersim/data/ai_001_002/_detail/cam_00/camera_keyframe_positions.hdf5'

with h5py.File(ori, 'r') as f_ori:
    ext_r = f_ori['dataset'][:]   # (N,3,3)

with h5py.File(pos, 'r') as f_pos:
...
extrinsics[:, :3, :3] = ext_r      # rotations
extrinsics[:, :3, 3]  = ext_t      # translations
extrinsics[:, 3, 3]   = 1.0
...
ranking, dists = compute_ranking(
    extrinsics,
    lambda_t=1.0,
    normalize=True,    # good default for Hypersim
    batched=True
)
