...
# depth-anything-3 package root in this repo layout:
# .../Depth-Anything-3/src/depth_anything_3/utils/reconstruct_npz_scene.py
#                                 ^ this directory must be on sys.path
DA3_SRC = Path(__file__).resolve().parents[2]
REPO_ROOT = DA3_SRC.parent
...
def _extract_depth(npz: np.lib.npyio.NpzFile) -> np.ndarray:
    if "pred_depth" in npz:
        depth = np.asarray(npz["pred_depth"], dtype=np.float64)
    elif "depth" in npz:
        depth = np.asarray(npz["depth"], dtype=np.float64)
    else:
        raise KeyError("Depth missing: expected `pred_depth` or `depth` in npz")
    if depth.ndim == 4 and depth.shape[1] == 1:
        depth = depth[:, 0]
    if depth.ndim != 3:
        raise ValueError(f"Depth must be (V,H,W) or (V,1,H,W), got: {depth.shape}")
    return depth


def _extract_rgb_u8(
    npz: np.lib.npyio.NpzFile,
    depth_for_fallback: np.ndarray | None = None,
) -> tuple[np.ndarray, str]:
    rgb_keys = ("pred_rgb", "rgb", "processed_images")
    for key in rgb_keys:
        if key not in npz:
            continue
        rgb = np.asarray(npz[key], dtype=np.float64)
        if rgb.ndim == 4 and rgb.shape[1] == 3:
            rgb = np.transpose(rgb, (0, 2, 3, 1))
        elif rgb.ndim == 4 and rgb.shape[-1] == 3:
            pass
        else:
            raise ValueError(f"{key} must be (V,3,H,W) or (V,H,W,3), got: {rgb.shape}")

        if rgb.dtype == np.uint8:
            return rgb.astype(np.uint8), key
        return (np.clip(rgb, 0.0, 1.0) * 255.0).astype(np.uint8), key

    if depth_for_fallback is None:
        raise KeyError("RGB missing: expected one of `pred_rgb`, `rgb`, `processed_images`")

    # Fallback for mini_npz without RGB: use normalized depth as grayscale texture.
    d = depth_for_fallback.astype(np.float64)
    d_min = np.nanmin(d, axis=(1, 2), keepdims=True)
    d_max = np.nanmax(d, axis=(1, 2), keepdims=True)
    denom = np.maximum(d_max - d_min, 1e-8)
    d_norm = (d - d_min) / denom
    gray = (np.clip(d_norm, 0.0, 1.0) * 255.0).astype(np.uint8)
    return np.repeat(gray[..., None], 3, axis=-1), "depth_grayscale_fallback"
...
def _extract_pred_intrinsics(npz: np.lib.npyio.NpzFile, views: int) -> np.ndarray:
    ...
    elif "intrinsics" in npz:
        pred_K = np.asarray(npz["intrinsics"], dtype=np.float64)
    else:
        raise KeyError("Pred intrinsics missing: expected `pred_K` or `intrinsics` in npz")

    if pred_K.ndim == 4 and pred_K.shape[0] == 1:
        pred_K = pred_K[0]
...
def _extract_pred_c2w(npz: np.lib.npyio.NpzFile, views: int) -> np.ndarray:
    ...
    elif "extrinsics" in npz:
        # mini_npz stores w2c in `extrinsics`; convert to c2w for this path.
        w2c = np.asarray(npz["extrinsics"], dtype=np.float64)
        if w2c.ndim == 4 and w2c.shape[0] == 1:
            w2c = w2c[0]
        w2c = _ensure_4x4(w2c)
        c2w = np.linalg.inv(w2c)
    else:
        raise KeyError("Pred poses missing: expected `pred_c2w` or `extrinsics` in npz")
...
def process_npz(npz_path: Path, args: argparse.Namespace) -> tuple[bool, str]:
    ...
    with np.load(npz_path, allow_pickle=True) as data:
        ...
        rgb_u8, texture_source = _extract_rgb_u8(data, depth_for_fallback=depth)
        cond_num = _cond_num(data)
        views, height, width = depth.shape

        is_mini_npz = (
            "depth" in data
            and "extrinsics" in data
            and "intrinsics" in data
            and "pred_depth" not in data
        )
        input_format = "mini_npz" if is_mini_npz else "raw_npz_or_compatible"

        if is_mini_npz and "conf" in data and conf is None:
            conf_arr = np.asarray(data["conf"], dtype=np.float32)
            if conf_arr.ndim == 4 and conf_arr.shape[1] == 1:
                conf_arr = conf_arr[:, 0]
            if conf_arr.ndim == 3 and conf_arr.shape[0] == views:
                conf = conf_arr
    ...
    has_rgb = texture_source != "depth_grayscale_fallback"
    print(
        f"[Recon] scene={sample_name} views={int(len(keep))} "
        f"depth=yes rgb={'yes' if has_rgb else 'no'} texture_source={texture_source} input_format={input_format}"
    )
