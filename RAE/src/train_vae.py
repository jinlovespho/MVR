...
def main():
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for train_step, batch in enumerate(train_loader):
            ...
            # if (
            #     rank == 0
            #     and len(full_cfg.data.val.list) != 0
            #     and training_cfg.vis.val_depth_every > 0
            #     and global_train_step % training_cfg.vis.val_depth_every == 0
            # ):
            ...
            #         # -------------------------------------------------
            #         # 1. Load batch (RAW tensors in [0,1])
            #         # -------------------------------------------------
            #         val_hq_raw = val_batch['hq_views'].to(device)  # (b v 3 h w)
            #         val_lq_raw = val_batch['lq_views'].to(device)  # (b v 3 h w)
            #         val_hq_id = val_batch['hq_ids']

            #         B, V, C, H_img, W_img = val_lq_raw.shape

            #         # -------------------------------------------------
            #         # 2. Prepare MODEL INPUT tensors
            #         # -------------------------------------------------
            #         # Flatten views
            #         val_lq_model = rearrange(val_lq_raw, 'b v c h w -> (b v) c h w')

            #         # Convert [0,1] → [-1,1] for VAE
            #         val_lq_model = val_lq_model * 2.0 - 1.0

            #         # VAE encode
            #         with torch.no_grad():
            #             latents = models['vae'].encode(val_lq_model).latent_dist.sample()
            #             latents = (
            #                 (latents - models['vae'].config.shift_factor)
            #                 * models['vae'].config.scaling_factor
            #             )

            #         # Restore shape
            #         val_lq_latent = rearrange(latents, '(b v) d h w -> b v d h w', b=B, v=V)

            #         # -------------------------------------------------
            #         # 3. Diffusion Sampling
            #         # -------------------------------------------------
            #         val_noise_generator.manual_seed(global_seed)
            #         noise = torch.randn_like(val_lq_latent)

            #         if full_cfg.mvrm.lq_latent_cond == 'addition':
            #             val_xt = noise + val_lq_latent
            #         elif full_cfg.mvrm.lq_latent_cond == 'concat':
            #             val_xt = torch.cat([noise, val_lq_latent], dim=2)

            #         with torch.no_grad():
            #             restored_samples = eval_sampler(val_xt, ema_model_fn)[-1]

            #         # If concat, split channels
            ...
            #         restored_latent = (
            #             restored_latent / models['vae'].config.scaling_factor
            #             + models['vae'].config.shift_factor
            #         )

            #         with torch.no_grad():
            #             decoded = models['vae'].decode(restored_latent).sample

            #         tmp1=((decoded + 1) / 2).clamp(0, 1)
            #         tmp2=((decoded + 1) / 2).cliop(0, 1)


            #         restored_images = ((decoded + 1) / 2).clamp(0, 1)
            #         restored_images = rearrange(
            #             restored_images, '(b v) c h w -> b v c h w', b=B, v=V
            #         )

            #         # -------------------------------------------------
            #         # 5. VISUALIZATION (use RAW tensors only)
            #         # -------------------------------------------------
            #         # RAW tensors are still in [0,1]
            #         hq_vis = val_hq_raw[0]             # (V,3,H,W)
            #         lq_vis = val_lq_raw[0]             # (V,3,H,W)
            #         restored_vis = restored_images[0]  # (V,3,H,W)

            #         # Stack vertically (HQ / LQ / Restored)
            #         final_vis = torch.cat([hq_vis, lq_vis, restored_vis], dim=2)

            #         # Save
            #         vis_dir = f"{experiment_dir}/vis_val_depth"
            #         os.makedirs(vis_dir, exist_ok=True)

            #         save_image(
            #             final_vis,
            #             f"{vis_dir}/{val_hq_id[0][0]}_step{global_train_step:07}.jpg"
            #         )
