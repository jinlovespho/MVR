...
def main():
    ...
    # load model 
    if full_cfg.analysis.encoder.name == 'da3':
        if full_cfg.analysis.encoder.size == 'GIANT':
            encoder = DepthAnything3.from_pretrained('depth-anything/DA3-GIANT-1.1').to(device)
            encoder.eval() 
        elif full_cfg.analysis.encoder.size == 'LARGE':
            encoder = DepthAnything3.from_pretrained('depth-anything/DA3-LARGE-1.1').to(device)
            encoder.eval() 
    else:
        raise ValueError
    ...
    # load data 
    if full_cfg.analysis.data.name == 'eth3d':
        img_root_path = full_cfg.analysis.data.eth3d.img_root_path 
        scene_names = ['courtyard', 'delivery_area', 'electro', 'facade', 'kicker', 'office', 'pipes', 'playground', 'relief', 'relief_2', 'terrains']
        datas=[]
