...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            inputs_embeds = self.model.embed_tokens(input_ids)  # b 156 3584
            if pixel_values is not None:
                pixel_values = pixel_values.type(self.visual.dtype) # 100 1176
                image_embeds = self.visual(pixel_values, grid_thw=image_grid_thw)   # 25 3584
                n_image_tokens = (input_ids == self.config.image_token_id).sum().item() # 25
                n_image_features = image_embeds.shape[0]    # 25
        ...
        # -----------------------------------------------------------------------
        #       set output_attentions=True for attention visulaization
        # -----------------------------------------------------------------------
        ...
        outputs = self.model(
            input_ids=None,
            position_ids=position_ids,                      # 3 1 156
            attention_mask=attention_mask,                  # b 156
            past_key_values=past_key_values,
            inputs_embeds=inputs_embeds,                    # b 156 3584
            use_cache=use_cache,                            # True
            output_attentions=output_attentions,            # False
            output_hidden_states=output_hidden_states,      # False
            return_dict=return_dict,                        # True
            cache_position=cache_position,                  # 156
        )
