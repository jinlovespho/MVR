...
def quat_to_rotmat(q):
    """
    q: (..., 4) quaternion in (qx, qy, qz, qw) order
    returns: (..., 3, 3)
    """
...
pos_file = '/mnt/dataset1/MV_Restoration/tartanair/data/abandonedfactory/Easy/P000/pose_left.txt'

poses = np.loadtxt(pos_file, dtype=np.float32)
# poses: (N,7)

t = poses[:, 0:3]          # (N,3)
q = poses[:, 3:7]          # (N,4)  qx qy qz qw
...
assert np.allclose(np.diag(dists), 0, atol=1e-6)

# Each camera ranks itself first
assert np.all(ranking[:,0] == np.arange(N))
